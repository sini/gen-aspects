...
#
# Here we override the composed tree by appending ONE thimble-level settings layer (a modules-only edit ⇒
# warm) and project the trace: the HABERDASHERY inventory (gen-modules/entities.nix, marked `pureModule`) is
# SPLICED unchanged (`haberdashery.thimbles` / `haberdashery.environments` ∈ `reused`), while the settings registry the
# edit lands on re-merges. This is "what the engine reused vs recomputed when we overrode the haberdashery's
...
let
  # The override edit: append a thimble-level settings override (bump prod-web-2's nginx workers). A plain
  ...
  settingsEdit = {
    config.scopeSettings."thimble:prod-web-2" = {
  ...
  # (gen-memo's `warmAdmits`) refuses warm, so `mode` flips to "cold" and the haberdashery leaves are NOT
...
in
{
  config.flake = {
    ...
    overrideHaberdasheryReused =
      builtins.elem "haberdashery.thimbles" trace.reused
      && builtins.elem "haberdashery.environments" trace.reused; # true
    ...
    overrideMarkedPureClean = trace.modules.clean != [ ]; # true — the marked-pure haberdashery is the sole clean entry
    ...
    overrideColdControlHaberdasheryDropped =
      !(builtins.elem "haberdashery.thimbles" (coldTrace.reused or [ ])); # true — nothing spliced on the cold path
  };
}
