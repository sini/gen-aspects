# The typed aspect SURFACE — declared PURELY inside the gen tree (value-injection).
#
...
#
# The caller (flake.nix) threads `genAspects`/`genMerge`/… into every tree module — the successor
# compose takes `specialArgs` caller-total, so the constructor merge is the caller's own act — and
# `lib` rides the same merge. The old `_module.args` gen-library threading is gone — reader-side
# libraries are provided as flake-parts `_module.args` in flake.nix (distinct from the injected
# VALUES).
{ genAspects, lib, ... }:
let
