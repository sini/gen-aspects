# gen-aspects demo: a web deployment over invented node kinds

A single integrated flake exercising 9 gen libraries together. Models a web deployment — environments (prod, staging, dev), `thimble` nodes, services, and observability, collected under the `haberdashery` registry — demonstrating aspects, schema extensions, scope-based settings composition, graph queries, selector patterns, policy dispatch, module bindings, and delivery-class realization.

The gen definition tree (`gen-modules/`) is composed **purely** by the [gen hub](https://github.com/sini/gen)'s successor compose (`gen.lib.compose`) — gen-merge's byte-mode `evalModuleTree`, not flake-parts' nixpkgs `lib.evalModules`. The flake injects the resolved config **values** into the flake-parts eval as the `genValues` module arg; the readers (`modules/*`) run the settings cascade + graph/selector/policy/bind demos over those values. No gen *type* ever enters the flake-parts options tree — the value-injection invariant that lets a gen aspect schema coexist with flake-parts (the old `options.schema`/`options.aspects`/`options.namespaces` embeds threw once moved onto the pure evaluator).

### Naming — invented kinds only

gen names no entities (ADR-0035), so a worked example uses an invented kind. The node kind here is `thimble` and the registry holding the environments and the thimbles is `haberdashery` — the vocabulary of the gen acceptance corpus, [gen-demo](https://github.com/sini/gen-demo). Aspect names (`services/nginx`, `firewall`, …) and settings values are the example's content, not gen vocabulary.
...
gen-modules/         — the gen definition tree, composed PURELY by gen.lib.compose (evalModuleTree)
  ...
  entities.nix       — haberdashery structure: environments + thimbles (marked `pureModule` — warm-reusable)
  aspects/
    ...
    basting.nix      — define-basting
  namespace.nix      — observability namespace: prometheus, grafana, loki
  settings.nix       — per-scope settings overrides (env-level, thimble-level)
modules/             — the flake-parts (reader) side; reads genValues, NOT composed into the tree
  ...
  terminal.nix       — the nixos-class TERMINAL: gen-delivery realize + the refinements layer → realized (per-thimble systems)
  override-trace.nix — warm override showcase: composed.override → the memoization trace (pureModule haberdashery reused)
...
- `project { selectNodes; }` — the node selector, required and without a default: it names `haberdashery.thimbles` as the node registry, and the projection needs each thimble tagged with the aspects its role runs (the thimble kind carries only env/role, so `selectNodes` synthesizes membership: web/all thimbles run `firewall` + `services/nginx`, database thimbles run `firewall` only).
- `realize { refinements; }` — the reader-computed per-`(thimble, aspect)` settings cascade (`composedSettings`) is passed as the per-node contribution layer (the declared layer order: projection < global < refinement). v0's `mkSystems` `wrapAll` bound only the resolved node instance, with no hook for that richer settings binding — so this demo used to hand-roll a "reader terminal" (per-aspect `genBind.wrap` + a stub `evalModules`). `realize`'s refinement layer lets the terminal itself wrap the class content.

The terminal here is a pure **DATA terminal** (the design's "attrset builder — no nixpkgs"): it wraps a thimble's class deferredModules with `genBind.wrapAll` and renders them through a bare `lib.evalModules` over stub options, so the demo asserts exact resolved values without a full NixOS eval. `realize` folds it per thimble into `realized.nixos.<thimble>` = the thimble's composed config.

**Intentional delta vs the reader terminal.** The reader terminal rendered each aspect in isolation; `realize` composes ALL of a thimble's aspects into ONE system. So `networking.firewall.allowedTCPPorts` on a web/all thimble now UNIONs the firewall cascade with nginx's public port (443) — the correct behaviour for a composed thimble system. The assertions moved from per-aspect equality to whole-system checks (cascade ports preserved as a subset; nginx contributes exactly its port; database thimbles, running firewall only, carry no 443).
...
| **gen-scope**    | Scope graph with env/thimble nodes, P-edges, neron traverse to collect settings in D > I > P order; `circular` (Kleene ascent) drives the policy dispatch convergence loop                                                                                                                            |
...
| **gen-bind**     | `wrapAll` binds the resolved per-thimble settings into the parametric NixOS modules at the terminal (via realize's `refinements` layer); `wrap`/`buildSignature` also drive the standalone binding-metadata probe (`bindings.nix`) with contract validation and provenance                            |
| **gen-delivery** | `project` discovers the DECLARED delivery classes per node (`cnf` + `selectNodes`) and reshapes the flat registry per thimble; `realize` folds each thimble's class content through the DATA terminal with the declared contribution-layer order                                                      |
...
- **Parametric** — `firewall`, `services.nginx`: a STATIC settings schema (introspectable by `flatten`/cascade) plus class content written as `{ settings, thimble, lib, ... }: { ... }` that CONSUMES resolved per-thimble settings, injected at the terminal via realize's `refinements` layer

### Settings cascade

Settings schemas declare defaults and merge strategies per field. Scope graph layers (thimble overrides > env overrides > aspect defaults) compose via `foldLayers`:

```
aspect default:  nginx.performance.workers = 4
env:prod:        nginx.performance.workers = 16
thimble:prod-web-1: nginx.performance.workers = 32   # ← wins
```
...
thimble:prod-web-1:  nginx.upstream.servers = ["app-1:3000", "app-2:3000", "app-3:3000"]
```
...
Parametric class content (`{ settings, thimble, lib, ... }: { ... }`) reads resolved
settings that don't exist until the cascade runs. `terminal.nix` closes the loop:
it passes the cascade's `composedSettings.<thimble>` (+ thimble) as realize's per-node
`refinements` layer, and `realize` folds each thimble's class deferredModules through
the DATA terminal (`genBind.wrapAll` + `lib.evalModules`) into `realized.nixos.<thimble>`.
...
dbHasNoNginxPort           # true — a database thimble runs firewall only (membership drives the build)
nginxInjectionResolved     # resolved workers=32 reaches nginx config (worker_processes 32)
```

### Cascade provenance + policy-overrides-thimble
...
workersProdWeb1Winner       # "thimble"    — negative control: policy doesn't touch workers
dbBackupSubkeyProvenance    # per-subkey on a recursive field:
                            #   { schedule="policy"; retention="policy";
                            #     method="thimble"; destination="thimble"; }
```
...
`warmDecision`. `override-trace.nix` overrides the composed tree by appending one thimble-level settings
...
The reuse turns on the `pureModule` marker: `gen-modules/entities.nix` wraps the haberdashery inventory in
...
overrideTrace.reused      # ["haberdashery.environments" "haberdashery.thimbles"] — the marked haberdashery, SPLICED
...
overrideHaberdasheryReused       # true  — haberdashery.thimbles ∈ reused (the pureModule's leaves)
overrideSettingsRemerged  # true  — scopeSettings re-merges as dirty-decl (settings.nix), regardless of the edit
overrideMarkedPureClean   # true  — the pureModule marking took effect (haberdashery is the sole clean entry)

overrideColdControlMode          # "cold" — RED control: the same edit + a second key (specialArgs) refuses warm
overrideColdControlHaberdasheryDropped  # true   — nothing spliced on the cold path (the invariants measure the warm path, not a constant)
```
