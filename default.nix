...
# rule. gen-prelude, gen-merge, gen-schema and gen-identity are all root inputs of the ci lock, so
# every path below is one segment long.
#
...
let
  ...
  # different node, and this repository's own ci lock is one of the seven where `gen-prelude`'s own
  # NODE KEY (`gen-prelude_3`) differs from the literal label `gen-prelude`, so that shortcut would
  # read a different revision here specifically, not a theoretical risk. IT TAKES ITS LOCK AS AN
  # ARGUMENT SO THAT THE ENTRY CELL CAN DRIVE THIS EXACT BINDING ON A FIXTURE WHERE THE TWO RULES
  # DISAGREE BY CONSTRUCTION; a resolver closed over this library's own lock could only ever be
  # compared against a second copy of itself. This is the ONE declaration of the rule in this
  # library — `ci/tests/entry.nix` reads this binding through the record the body hands `wire`,
  # instead of transcribing the fold a second time.
  resolve =
    lock:
    let
      following =
...
in
{
  ...
  # The one minting authority: a dependency-free leaf, so its lib is a bare value and `dep`'s arity
...
# it never enters the lambda that binds `prelude`/`merge`/`schema`/`identity`, and
# `builtins.deepSeq` on the flake's exported surface cannot make up the difference either — it does
# not enter a lambda, and the standalone path is not wrapped in it at all. With the eager body a
# WHNF force of the root reaches all four, whatever the published surface's shape.
#
