...
| Kind/registry infrastructure. gen-aspects now consumes exactly ONE gen-schema name — the mint moved to `gen-identity` and arrives as its own injected input, so the count went DOWN by one. What remains from gen-schema is (`lib/default.nix`, the `inherit (schema) hashIdentity` passed into `types.nix`) and `mkSchemaOption` (`lib/schema.nix`'s `schemaOpt`) | `gen-schema` — "gen-schema: typed record registry with extension points for the pure-gen module system" |
...
| An **opaque body is a third thing besides a closure and a `deferredModule`**: one the structural function-scan REFUSED, being cyclic or nested past its depth budget. A derivation answers by **type test before any descent** | `lib/identity.nix`'s `hasFn` (`hasFnMaxDepth`, `hasFnDepthRefusal`) under `bodyKey`. Each of the three used to abort `stack overflow; max-call-depth exceeded` through `guardKey`, escaping `tryEval`; now a cyclic body ⇒ `guard-loc:<anon>` and a derivation body ⇒ `guard:always:a47e4ab7…`. Controls same run: a deep finite body still mints `guard:always:b75e2b04…`, a lambda-carrying one still ⇒ `guard-loc:<anon>`. A value whose own thunk is a black hole (`[ 1 ] ++ l`) is **outside** this — it aborts when forced, before any walk. Tests: `test-cyclic-attrset-refused-with-live-controls`, `test-derivation-short-circuits-before-descent` |
| `aspectId` routes through `gen-identity`'s `hashIdentity` over `[ origin, key ]`, **not** `mkIdentityModule` — so `description` is not in the preimage | `lib/types.nix`'s `aspectId`; `aspectId [ ] a == genSchema.hashIdentity "aspect" [ "origin" "key" ] …` ⇒ `true`, and two aspects differing only in `description` ⇒ same id. `git grep -n mkIdentityModule -- lib/` ⇒ no hits; positive control `git grep -n hashIdentity -- lib/` ⇒ 5 hits. Tests: `test-aspectid-is-canonical-formula`, `test-description-not-in-id` |
