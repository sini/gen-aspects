...
in
{
  ...
  flake.tests.entry.test-control-the-compared-surface-is-non-trivial = {
    expr = builtins.length (builtins.attrNames standalone) > 15;
    expected = true;
  };
  ...
  flake.tests.entry.test-control-the-comparison-discriminates = {
    expr = builtins.attrNames standalone == builtins.attrNames (removeAttrs aspects [ "key" ]);
    expected = false;
  };
  ...
  # aspect-path derivation runs a `prelude` combinator, so a shim forwarding something that is not
  # gen-prelude fails here rather than passing quietly.
  flake.tests.entry.test-the-shims-library-is-live = {
    expr = standalone.pathKey [
      "a"
      "b"
    ];
    expected = "a/b";
