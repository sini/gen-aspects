...
#
# Every other cell here takes `aspects` from `ci/flake.nix`, which builds it with `import ../lib`
...
let
  ...
  # does: `gen-prelude`'s node key is `gen-prelude_3`, not the label `gen-prelude`. Reading the lock
...
in
{
  ...
  # The two entry paths are ONE library. `aspects` is built from ci's flake inputs, `dispatched`
  ...
  # aspects` is not the alternative either — `./lib`'s surface carries function-valued members, and
  ...
  flake.tests.entry.test-the-defaulted-entry-publishes-the-flake-surface = {
    ...
    expected = builtins.attrNames aspects;
  };
  ...
  # fold in `default.nix` with the shortcut and this reds — and at THIS library, unlike most, the
  # cell above would too, since the real lock also disagrees at `gen-prelude`.
